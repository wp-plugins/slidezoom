		...
		case "zip":
			...
			for ($i=0; $i < $FileCount; $i++) {
				...
				if ( @intval($AryOptions['ThumbMode']) === 2 ) {
					...
					$wud = wp_upload_dir();
					$image = new clsImage();
					$image -> load($wud['path']."/".sanitize_file_name($file['name']));
...
print <<<FILE_INPUT
     		<label id="lblUploadID">Image: <input type="file" id="FileUpload[]" name="FileUpload[]" size="50"/><br/></label>
FILE_INPUT;
		}
